@extends('layouts.user')

{{-- Web site Title --}}
@section('title')
    {{ $title }}
@stop

@section('styles')
    <link rel="stylesheet" href="{{ asset('css/tagsinput.css') }}" type="text/css">
@stop
{{-- Content --}}
@section('content')
    @include('vendor.flash.message')
    <div class="page-header clearfix">
        <div class="pull-right">
            <a href="{{ url($type.'/eventRoomCreate') }}" class="btn btn-primary">
                <i class="fa fa-plus-circle"></i> {{ trans('eventSetting.create') }}</a>
        </div>
    </div>
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title">
                <i class="material-icons">airline_seat_individual_suite</i>
                {{ $title }}
            </h4>
            <span class="pull-right">
            <i class="fa fa-fw fa-chevron-up clickable"></i>
            </span>
        </div>

        <div class="panel-body">
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        <div class="controls">
                            {!! Form::select('hotel',$hotels, (isset($event) ? $event->booking->location_id : null), ['id'=>'hotel', 'class' => 'form-control select2','onchange'=>'filter1(this.value)']) !!}
                        </div>
                    </div>
                </div>
            </div>
            <input type="hidden" value="" id="idData">
            <div class="table-responsive">
                <table class="table table-bordered table-hover" id="data">
                    <thead>
                    <tr>
                        <th>{{ trans('eventSetting.hotelName') }}</th>
                        <th>{{ trans('eventSetting.room') }}</th>
                        <th>{{ trans('eventSetting.actions') }}</th>
                    </tr>
                    </thead>
                    <tbody>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
@stop

@section('scripts')
    <script>
        $(function() {
            $("#hotel").select2({
                theme: "bootstrap",
                placeholder: "{{ trans('eventSetting.hotelName') }}"
            });
        });

        function filter1(val){
            oTable.ajax.url('{!! url($type.'/eventRoomData') !!}/' + val);
            oTable.ajax.reload();
        }
    </script>
@stop
